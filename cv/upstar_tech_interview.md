...
- [Методи масивів (map, filter, reduce...)](#7-методи-масивів--map-filter-reduce-foreach-find-some-every)
...
**7. "Як зробити однопоточний JavaScript багатопоточним?"**

"JavaScript однопоточний — один event loop, один call stack. Але є механізм для паралельних обчислень — Web Workers у браузері і Worker Threads у Node.js. Worker — це окремий потік зі своїм event loop. Він не має доступу до DOM і не ділить пам'ять з основним потоком. Спілкуються через postMessage / onmessage — передають дані копіюванням або через SharedArrayBuffer.

Коли це потрібно: парсинг великого JSON, обробка зображень, складні обчислення які б заблокували UI або основний серверний потік. У контексті SSR — якщо рендер важкий, можна винести його в worker thread щоб не блокувати event loop і не тримати інших користувачів.

Є ще Service Workers — але це не про багатопоточність, а про перехоплення мережевих запитів, кешування і офлайн-роботу."

**8. "Як фіксити повільний TTFB?"**

"TTFB — Time To First Byte — час від запиту до першого байта відповіді від сервера. Якщо TTFB повільний, причини зазвичай: сервер довго рендерить HTML при SSR, повільні API-виклики які сервер чекає перед віддачею, або відсутній кеш.

Фікси: кешування відрендереного HTML на рівні CDN або серверного кешу з stale-while-revalidate — віддаємо старий кеш одразу, а свіжий генеруємо у фоні. Оптимізація серверних запитів — паралельні замість послідовних. Streaming SSR — починаємо віддавати HTML ще до завершення повного рендеру, браузер вже малює шапку поки сервер ще рендерить контент. Таймаути на зовнішні API — якщо щось не відповіло за 3 секунди, віддаємо fallback а не чекаємо вічно."

**9. "Методи масивів — map, filter, reduce, forEach, find, some, every?"**

"Це основні інструменти для роботи з масивами у функціональному стилі — без мутацій, без ручних циклів.

**map** — трансформує кожен елемент і повертає новий масив тієї ж довжини. Наприклад, з масиву юзерів витягнути тільки імена: `users.map(u => u.name)`. Оригінал не чіпає.

**filter** — повертає новий масив тільки з елементами які пройшли умову. `users.filter(u => u.isActive)` — отримаю тільки активних. Теж без мутації.

**find** — повертає перший елемент який підійшов під умову, або undefined. `users.find(u => u.id === targetId)`. Зупиняється одразу як знайшов — не проходить весь масив.

**some** — повертає true якщо хоча б один елемент підходить: `errors.some(e => e.critical)`. Зупиняється на першому збігу.

**every** — повертає true тільки якщо всі елементи підходять: `fields.every(f => f.isValid)`. Зупиняється на першому який не пройшов.

**forEach** — просто прохід по масиву, нічого не повертає. Використовую рідко — переважно коли потрібен побічний ефект, наприклад відправити аналітику. Для трансформацій завжди map або filter.

**reduce** — найпотужніший, зводить масив до одного значення. Класика — підрахувати суму: `items.reduce((sum, item) => sum + item.price, 0)`. Але з ним легко написати нечитабельний код, тому використовую обережно. Якщо reduce стає складним — краще розбити на map + filter.

**Практичне правило:** map/filter/find покривають 90% потреб. reduce — для агрегацій. forEach — крайній випадок. Головне — не мутувати оригінальний масив. І якщо працюю з даними від API — спочатку перевіряю що це масив: `(items ?? []).map(...)`, бо null.map() впаде."
